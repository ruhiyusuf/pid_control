...
def _sweep_worker():
    try:
        step_log = []
        t_cursor = 0.0  # expected time cursor (relative to sweep start)
        ...
        for idx, p in enumerate(steps, start=1):
            ...
            step_log.append({
                "step_idx": idx,
                "params": {k: float(p[k]) for k in ["v_low","v_high","v_mid","t_hold_high","t_hold_mid"]},
                "t_start_expected_s": float(t_cursor),
                "t_end_expected_s": float(t_cursor + wait_s),
                "cycle_duration_s": float(dur_cycle),
                "repeats": int(max(1, REPEAT_CYCLES_PER_STEP)),
                "extra_dwell_s": float(EXTRA_DWELL_BETWEEN_STEPS_S),
            })
            t_cursor += wait_s + float(EXTRA_DWELL_BETWEEN_STEPS_S)
        ...
        out = {
            ...
            "settings": {
                ...
                "EXTRA_DWELL_BETWEEN_STEPS_S": float(EXTRA_DWELL_BETWEEN_STEPS_S),
                "slope_ref_hz": float(params.get("slope_ref_hz", 0.0)),
                "slope_scale": float(params.get("slope_scale", 1.0)),
            },
            "step_log": step_log,
        }
        with open(f"sweep_run_{run_id}.json", "w") as f:
            json.dump(out, f, indent=2)

        dpg.set_value("status_text", f"Finished. Saved sweep_run_{run_id}.json")
...
def create_start_button_theme():
    with dpg.theme() as start_btn_theme:
        with dpg.theme_component(dpg.mvButton):
            dpg.add_theme_color(dpg.mvThemeCol_Button, (40, 120, 60), category=dpg.mvThemeCat_Core)
            dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (55, 150, 80), category=dpg.mvThemeCat_Core)
            dpg.add_theme_color(dpg.mvThemeCol_ButtonActive, (30, 95, 50), category=dpg.mvThemeCat_Core)
            dpg.add_theme_style(dpg.mvStyleVar_FrameRounding, 6, category=dpg.mvThemeCat_Core)
            dpg.add_theme_style(dpg.mvStyleVar_FramePadding, 16, 10, category=dpg.mvThemeCat_Core)
    return start_btn_theme
...
def add_waveform_inputs(tab_id, values):
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(width=360, height=430, border=True):
            dpg.add_text(f"{'Initial' if tab_id == 1 else 'Final'} waveform settings")
            ...
            for key in ["v_low", "v_high", "v_mid"]:
                dpg.add_input_float(
                    ...
                    width=180,
                    callback=lambda s, a, tid=tab_id: (
                        update_preview(tid),
                        update_sweep_time_ui(),
                        update_sweep_preview_ui()
                    ),
            ...
            with dpg.group(tag=f"time_group_{tab_id}"):
                dpg.add_text("Timing (selected time units)")
                for key in ["t_hold_high", "t_hold_mid"]:
                    dpg.add_input_float(
                        label=key,
                        tag=f"{key}_{tab_id}",
        ...
        # ---------------- RIGHT: plot ----------------
        with dpg.child_window(width=930, height=430, border=True):
            plot_label = "Initial Preview" if tab_id == 1 else "Final Preview"
            plot_series = "plot1_series" if tab_id == 1 else "plot2_series"
            axis_tag = "axis1" if tab_id == 1 else "axis2"

            with dpg.plot(label=plot_label, height=390, width=-1):
                dpg.add_plot_axis(dpg.mvXAxis)
                dpg.add_plot_axis(dpg.mvYAxis, tag=axis_tag)
                dpg.add_line_series([], [], parent=axis_tag, tag=plot_series)
                dpg.set_axis_limits(axis_tag, y_min, y_max)


def add_increment_inputs():
    with dpg.group(horizontal=True):
        with dpg.child_window(width=320, height=180, border=True, tag="inc_voltage_group"):
        ...
        with dpg.child_window(width=320, height=180, border=True, tag="inc_time_group"):
            dpg.add_text("Time increments (selected units)")
            dpg.add_spacer(height=4)
            for k in ["t_hold_high", "t_hold_mid"]:
                dpg.add_input_float(
                    label=f"inc_{k}",
...
def build_gui():
    dpg.create_context()
    start_btn_theme = create_start_button_theme()
    with dpg.window(label="RP Sweep GUI (step-by-step deploy + sweep preview)",
                    width=1550, height=980):

        # =====================================================
        # TOP ROW: file / hardware / mode
        # =====================================================
        with dpg.group(horizontal=True):
            ...
            with dpg.child_window(width=420, height=100, border=True):
                dpg.add_text("Hardware")
                dpg.add_input_text(
                    label="Red Pitaya IP",
                    tag="rp_ip",
                    default_value=str(params["rp_ip"]),
                    width=240
                )
                dpg.add_combo(
        ...
        # =====================================================
        # SECOND ROW: timing / slope / deploy / summary
        # =====================================================
        with dpg.group(horizontal=True):

            with dpg.child_window(width=340, height=160, border=True, tag="global_timing_group"):
                dpg.add_text("Global timing")
                dpg.add_text("(used when sweeping voltage only)")
                dpg.add_spacer(height=4)
            ...
            with dpg.child_window(width=300, height=160, border=True):
                dpg.add_text("Slope reference")
                dpg.add_text("(defines ramp slope only)")
                dpg.add_spacer(height=4)
            ...
            with dpg.child_window(width=420, height=160, border=True):
                dpg.add_text("Step deploy settings")
                dpg.add_spacer(height=4)

                dpg.add_input_int(
                    label="Repeat cycles per step",
                    tag="repeat_cycles_ui",
                    default_value=int(REPEAT_CYCLES_PER_STEP),
                    width=140,
                    callback=lambda s, a: (
                        update_sweep_time_ui(),
                        update_sweep_preview_ui()
                ...
                dpg.add_input_int(
                    ...
                    tag="samples_per_seg_ui",
                    default_value=int(SAMPLES_PER_SEG),
                    width=140,
                    callback=lambda s, a: (
                        update_sweep_time_ui(),
                        update_sweep_preview_ui()
                ...
                dpg.add_input_float(
                    label="Extra dwell between steps (s)",
                    tag="extra_dwell_ui",
                    default_value=float(EXTRA_DWELL_BETWEEN_STEPS_S),
                    width=160,
                    format="%.3f",
                    callback=lambda s, a: (
                        update_sweep_time_ui(),
            ...
            with dpg.child_window(width=440, height=160, border=True):
                dpg.add_text("Sweep summary")
                dpg.add_spacer(height=4)
        ...
        with dpg.group(horizontal=True):
            ...
            with dpg.child_window(width=450, height=120, border=True):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_button(label="Regenerate Preview", callback=regenerate_sweep_preview, width=150, height=32)
                    dpg.add_button(label="Start Sweep", tag="start_sweep_btn", callback=start_sweep, width=170, height=40)
                    dpg.add_button(label="Stop", callback=stop_sweep, width=90, height=32)
                    dpg.bind_item_theme("start_sweep_btn", start_btn_theme)
            with dpg.child_window(width=370, height=120, border=True):
                dpg.add_text("Status")
                dpg.add_text("", tag="status_text")

            with dpg.child_window(width=700, height=120, border=True):
        ...
        # =====================================================
        # FOURTH ROW: increments
        # =====================================================
        with dpg.collapsing_header(label="Sweep increments", default_open=True):
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Sweep Preview"):
                with dpg.group(horizontal=True):
                    with dpg.child_window(width=280, height=500, border=True):
                        dpg.add_text("Sweep Preview")
                        dpg.add_spacer(height=4)
                        dpg.add_text("This shows the actual stepwise sweep")
                        dpg.add_text("that is being sent to the RP.")
                        dpg.add_spacer(height=8)
                        dpg.add_text("", tag="sweep_preview_status")

                    with dpg.child_window(width=1230, height=500, border=True):
                        with dpg.plot(label="Sweep Preview (downsampled)", height=455, width=-1):
    ...
    dpg.create_viewport(title="RP Sweep GUI", width=1600, height=1040)
    dpg.setup_dearpygui()
    dpg.show_viewport()
