...
params = {
    ...
    "start_v_ms": 20.0, "slope_up_ms": 20.0, "end_v_ms": 500.0,
    "rp_ip": "rp-f0cbdd.local", "output_channel": 1
}
...
def deploy_waveform():
    try:
        ...
        duration_s = t_waveform[-1]  # last timestamp IS total duration

        ch = int(params["output_channel"])
        assert ch in [1, 2]

        # Convert waveform to int16
        wave_i16 = (np.array(waveform) * (2**13 - 1)).astype(np.int16)
        ...
        with socket.create_connection((RP_IP, RP_PORT), timeout=5) as s:
            # Send header
            s.sendall(struct.pack(">I", total_bytes))

            s.sendall(struct.pack(">B", ch))   
            # Send duration as float32
            s.sendall(struct.pack(">f", duration_s))
            # Send waveform data
            s.sendall(payload)

        dpg.set_value("status_text", f"Waveform deployed ({duration_s:.3f} s)")
...
with dpg.window(label="Waveform Generator", width=1000, height=800):    
    ...
    with dpg.collapsing_header(label="Duration"):
        with dpg.table(header_row=True):
            ...
            dpg.add_table_column(label="Value")
            dpg.add_table_column(label="Increment")
            for key in ["start_v_ms", "slope_up_ms", "end_v_ms"]:
                with dpg.table_row():
                    dpg.add_text(key.replace("_", " ").title())
                    dpg.add_input_float(tag=key, default_value=params[key], step=increments[key], callback=update_parameters)
